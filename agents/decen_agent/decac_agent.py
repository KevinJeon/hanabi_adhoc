...
class PolicyNet(nn.Module):
  def __init__(self,num_state,num_action,num_hidden,num_belief):
    ...
    self.fc1_v = nn.Linear(num_state+num_belief, num_hidden)
    self.fc2 = nn.Linear(num_hidden, num_hidden)
    self.fc2_v = nn.Linear(num_hidden, num_hidden)
    self.fc3 = nn.Linear(num_hidden, num_action)
    self.fc3_v = nn.Linear(num_hidden, 1)
...
class DBPLAgent(object):
  def __init__(self,num_state,num_action,num_hidden,num_belief,num_player,log_dir):
    ...
    self.b_optim = Adam(self.belief_module.parameters(),lr=10e-4)
    self.p_optim = [Adam(self.policy_modules[i].parameters(),lr=10e-4,betas=(0.9,0.999)) for i in range(num_player)]
    self.gamma = 0.99
    self.epsilon = 0.2
    self.k = 3
    self.batch_size = 64
    self.iter = [0]*2
  def begin_episode(self,current_player,legal_moves,observation_vector,step_number):
    # train step?
    self._train()
    self.init_belief()
    ...
    self.action,logprobs = self._select_action(current_player,observation_vector,
                                               self.belief[current_player],legal_moves,step_number)
    self.memory.add_count += 1
    return self.action,logprobs,self.belief[current_player],self.c[current_player].cpu().detach().numpy()

  def step(self,reward,current_player,legal_moves,observation_vector,step_number):
    self._train()
    # train step
    self.action,logprobs  = self._select_action(current_player,observation_vector,
                                                self.belief[current_player],legal_moves,step_number)
  ...
  def _train(self,end=False):
    ...
    if (self.memory.add_count >= self.batch_size) or (end == True):
      ...
      for i in range(self.num_player):
        ...
        self.old_policy_modules[i].load_state_dict(self.policy_modules[i].state_dict())
      self.memory.clear()
  def _select_action(self,current_player,observation,belief,legal_actions,step):
    ...
    p = p.cpu().detach().view(-1)
    p[legal_actions==float('-inf')] = 0
    dist = Categorical(p)
    action = dist.sample()
  ...
  def update_others_belief(self,current_player,all_observations,action):
    for n,obs in enumerate(all_observations):
      ...
      one_hot_action = one_hot_action.view(1,-1)
