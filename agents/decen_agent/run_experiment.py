...
@gin.configurable
def create_agent(environment, obs_stacker, agent_type='DQN',log_dir='1'):
  ...
  if agent_type == 'DQN':
    ...
    return decac_agent.DBPLAgent(
        num_state=obs_stacker.observation_size(),
        num_hidden=512,
        num_belief=125,
        num_action=environment.num_moves(),
        num_player=environment.players,
        log_dir='./logs/{}'.format(log_dir))
  else:
...
def parse_observations(observations, num_actions, obs_stacker):
  ...
  all_observations = []
  for n,obs in enumerate(observations['player_observations']):
      all_observations.append(obs['vectorized'])
  return current_player, legal_moves, observation_vector,all_observations
...
def run_one_episode(agent, environment, obs_stacker):
  ...
  step_number = 0
  current_player, legal_moves, observation_vector,all_observations = (
      parse_observations(observations, environment.num_moves(), obs_stacker))
  action,logprobs,belief,confidence = agent.begin_episode(current_player, legal_moves, observation_vector,step_number)
  agent.save_all(action, all_observations)
  agent.update_others_belief(current_player,all_observations,action)
  is_done = False
  total_reward = 0
  ...
  while not is_done:
    ...
    agent.memory.add(current_player,np.array(observation_vector,dtype=np.uint8,copy=True),
                     np.eye(environment.num_moves())[action.item()],reward,is_done,legal_moves,
                     belief,logprobs,confidence)
    ...
    if current_player in has_played:
      action,logprobs,belief,confidence = agent.step(reward_since_last_action[current_player],
                          current_player, legal_moves, observation_vector,step_number)
    else:
      # Each player begins the episode on their first turn (which may not be
      # the first move of the game).
      action,logprobs,belief,confidence = agent.begin_episode(current_player, legal_moves,
                                   observation_vector,step_number)
